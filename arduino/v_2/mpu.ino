...
void mpu_init(MPU6050 mpu, int interrupt_pin){
  ...
  // MATT: I think these are the sensitivites for us. Could be wrong
  //    mpu.setZAccelOffset(16384);
  //    mpu.setXAccelOffset(16384);
  //    mpu.setYAccelOffset(16384);
...
void doubleTrapExecution() {
  ...
  Serial.printf("a: %d\n", aaReal.z);
  Serial.printf("az: %d\n", /*aaReal.z -*/ velocity_x.zero);

  TrapezoidApproximate(&velocity_x, cur_time, aaReal.z/1000);
  TrapezoidApproximate(&position_x, cur_time, velocity_x.value);

  Serial.printf("x: %d\n", position_x.value);
  Serial.printf("v: %d\n", velocity_x.value);
}

void TrapezoidApproximate(TrapezoidRule * t, float new_x, float new_y) {
   t->value += 0.5*(new_x - (t->last_x))*((new_y - t->zero) + (t->last_y - t->zero));
